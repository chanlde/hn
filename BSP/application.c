...
#include "custom_serial/custom_serial.h"
...
#ifndef PSDK_HEAP_LOG
#define PSDK_HEAP_LOG 0
#endif

#if PSDK_HEAP_LOG
static void DjiUser_LogHeapTag(const char *tag)
{
    USER_LOG_INFO("[heap] %s free=%lu", tag != NULL ? tag : "?", (unsigned long)xPortGetFreeHeapSize());
}
#else
#define DjiUser_LogHeapTag(tag) ((void)0)
#endif
...
{
    return s_isApplicationStart ? 1 : 0;
}
...
//Attention: if you want to run payload sdk on extension port, please define the macro USE_USB_HOST_UART.
...

    {
        ...
        USER_LOG_INFO("Device ID = %s", deviceId);
        USER_LOG_INFO("Firmware Version = %u.%u.%u.%u inner=%u",
                      USER_FIRMWARE_MAJOR_VERSION,
                      USER_FIRMWARE_MINOR_VERSION,
                      USER_FIRMWARE_MODIFY_VERSION,
                      USER_FIRMWARE_DEBUG_VERSION,
                      USER_FIRMWARE_INNER_VERSION);
        returnCode = DjiCore_SetSerialNumber(deviceId);
    }
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    DjiUser_LogHeapTag("after SetAlias");
    ...
    DjiUser_LogHeapTag("before WidgetStartService");
    returnCode = DjiTest_WidgetStartService();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("widget sample init error");
    }
    DjiUser_LogHeapTag("after WidgetStartService");
#endif
    ...
    DjiUser_LogHeapTag("before DjiCore_ApplicationStart");
    returnCode = DjiCore_ApplicationStart();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("start sdk application error");
    }
    DjiUser_LogHeapTag("after DjiCore_ApplicationStart");

    if (CustomSerial_StorageInit() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("custom_serial state init error");
    }
    DjiUser_LogHeapTag("after CustomSerial state init");

    s_isApplicationStart = true;
    DjiUser_LogHeapTag("PSDK application started (MQTT may proceed)");
