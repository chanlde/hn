...
public class AMapPlaceDelegate implements IInternalPlacesClient, PoiSearch.OnPoiSearchListener {
    ...
    public void regeocodeSearchAsyn(DJILatLng latLng) {
        try {
            ...
            RegeocodeQuery querys = new RegeocodeQuery(
                    new LatLonPoint(latLng.getLatitude(), latLng.getLongitude()), POI_RADIUS, GeocodeSearch.AMAP);
            ...
            geoCoderSearch.setOnGeocodeSearchListener(new GeocodeSearch.OnGeocodeSearchListener() {
                ...
                public void onRegeocodeSearched(RegeocodeResult regeocodeResult, int resultId) {
                    if (resultId == 1000) {
                        ...
                        result.setSubStreet(regeocodeAddress.getStreetNumber() != null ?
                                regeocodeAddress.getStreetNumber().getStreet() : null);
            ...
            e.printStackTrace();
            // 这里可以记录日志，或者提示用户
            Log.e("AMapException", "An error occurred while performing reverse geocode search: " + e.getMessage());
        }
    }
}